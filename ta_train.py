...
class CFG:
    ...
    weight_decay = 1e-6
    edge_smooth_value = 1
    edge_smooth_depth = 0 # Num of pixels smoothed
    output_dir = './deeplabv3_rn50_seed_0'
...
def prepare_loaders(df, fold):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=CFG.batch_size,
                              num_workers=CFG.num_workers, shuffle=True, pin_memory=False, drop_last=True)
...
class MyoVisionUS_Dataset(torch.utils.data.Dataset):
    def __init__(self, df, labeled=True, transforms=None, color_palette=CFG.color_palette):
        ...
        self.u_groups = self.df.groups.unique()
        
    def __len__(self):
        return len(self.u_groups)
    
    def __getitem__(self, index):
        group = self.df[self.df.groups == self.u_groups[index]].reset_index(drop=True)
        row_idx = np.random.randint(0, len(group))
...
def run_training(model, optimizer, scheduler, train_loader, valid_loader, device, num_epochs, fold):
    ...
    for epoch in range(1, num_epochs + 1): 
        ...
        # deep copy the model
        if (val_dice+val_jaccard)/2 >= best_score:
            print(f"{c_}Valid Score Improved ({best_score:0.4f} ---> {(val_dice+val_jaccard)/2:0.4f})")
            best_score   = (val_dice+val_jaccard)/2
            best_dice    = val_dice
    ...
    print("Best Dice Score: {:.4f} and IoU: {:.4f} at Epoch {:}".format(best_dice, best_jaccard, best_epoch))
...
def main():
    set_seed(CFG.seed)
    df = pd.DataFrame(glob.glob("dataset_path/*.png"))
    df['base_path'] = df['label_path'].apply(lambda x: x.split('/')[-1])
    df['img_path'] = df['label_path'].apply(lambda x:  x.replace('trainingLabels', 'trainingImages').replace('labels', ''))
    df['groups'] = df['base_path'].apply(lambda x: make_groups(x))

    kf = GroupKFold(n_splits=CFG.n_folds)
    for fold, (train_idx, val_idx) in enumerate(kf.split(df, groups=df['groups'])):
    ...
    for fold in CFG.folds_to_run:
        ...
        model, history = run_training(model, optimizer, scheduler, train_loader, valid_loader, CFG.device, CFG.epochs, fold)

    log_df = pd.DataFrame.from_dict(history)
    log_df.to_csv(f'{CFG.output_dir}/{fold}_log.csv', index=False)

    trace = torch.jit.trace(model, torch.randn(1, 3, 512, 512).to(CFG.device))
    torch.jit.save(trace, f'{CFG.output_dir}/best_model_{fold}.pt')
